...
import { getPlanAllData } from "../../Redux/Action/Action";
...
const Plans = (props) => {
    ...
    const categoryData = useSelector((state) => state.planCategoryReducer.planCategory);
    ...
    function search(items) {
        return items?.filter((item) =>
            ...
            .filter((item) =>
                Object.values(item).some((val) =>
                    String(val).toLowerCase().includes(props.catePlanSearch.toLowerCase())
                )
            );
    }
    ...
    const handleUploadSubmit = (e) => {
        ...
        if(!uploadDoc.file_name){
            document.getElementById("file_name_err").innerHTML="* File name cannot be left empty";
        }
        if(!uploadDoc.location){
            toast.error("Please select a Location");
        }
        if(!uploadDoc.upload_document){
            document.getElementById("upload_document_err").innerHTML="* A document must be uploaded";
        }
        if(!uploadDoc.date){
            document.getElementById("date_err").innerHTML="* Date cannot be left empty";
        }
        if(!uploadDoc.category){
            document.getElementById("category_err").innerHTML="* Category cannot be left empty";
        }
        if(uploadDoc.file_name && uploadDoc.location && uploadDoc.upload_document && uploadDoc.date && uploadDoc.category){
                                                                ...
                                                                <>
                                                                    {
                                                                        curElem.category == cateCurelem._id ? (
                                                                            <li className='documentTableItem'
                                                                                ...
                                                                                onClick={(event) => {
                                                                                    switch (event.detail) {
                                                                                        ...
                                                                                        // }
                                                                                        case 2: {
                                                                                            window.open(process.env.REACT_APP_IMAGE_BASE_URL+`/${curElem.upload_document}`);
                                                                                            break;
                                                                                        }
                                                                                        default: {
                                                                                            break;
                                                                            ...
                                                                            >
                                                                                ...
                                                                                <div>{(curElem.calendar_reminder_interval) ? (
                                                                                    <div class="ctm-checkbox"><label class="ctm-checkbox-container"><input type="checkbox" checked readOnly /><span class="ctm-checkbox-checkmark"></span></label></div>
                                                                                ) : "-"}</div>
                                                                                <div>{(curElem.note_item) ? (
                                                                                    <div class="ctm-checkbox"><label class="ctm-checkbox-container"><input type="checkbox" checked readOnly /><span class="ctm-checkbox-checkmark"></span></label></div>
                                                                                ) : "-"}</div>
                                                                                {
                                                                                    (contMenuActve && getSelectedId == curElem._id) && (
                                                                                        <>
                                                                                            <div className='btn-group' style={{
                                                                                                // top: points.top,
                                                                                                // left: points.left,
                                                                                            }}>
                                                                                                <button className='btn cmn_yellow_bg' onClick={(e) => { categoryListAPI(); singleDocumentAPI(curElem._id); props.setEditCat(true) }}>
                                                                                                    <EditOutlinedIcon className="edit " color="action" />
                                                                                                    ändern</button>
                                                                                                <button className='btn cmn_red_bg' onClick={(e) => { deleteDocument(curElem._id) }}>
                                                                                                    <DeleteOutlinedIcon className="Delete " color="action" />
                                                                                                    Delete</button>
                                                                                            </div>
                                                                                        </>
                                                                                    )
                                                        ...
                                                        <div className='col-md-6'>
                                                            ...
                                                            <div className='form-group'>
                                                                ...
                                                                <select name="category" id="category" className='form-control' onChange={(e) => { handleUploadChange(e) }} value={uploadDoc.category}>
                                                                    ...
                                                                    {
                                                                        categoryData.length > 0 && categoryData?.map((curelem, index) => {
                                                                            return (
                                                        ...
                                                        <div className='col-md-6'>
                                                            <div className='uploadFileArea'>
                                                                <div className="form-group file-area">
                                                                    ...
                                                                    <div className="errors" id="upload_document_err"></div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
